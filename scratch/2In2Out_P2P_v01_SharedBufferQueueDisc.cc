...
std::string queue_disc_type = "SharedBufferQueueDisc_v01"; 
...
void
TrafficControlPacketsInQueueTrace (uint32_t oldValue, uint32_t newValue)
{
  std::cout << "TrafficControllPacketsInQueue " << ": " << newValue << std::endl;
}
...
// void
// DevicePacketsInQueueTrace0 (uint32_t oldValue, uint32_t newValue)
// {
//   std::cout << "PacketsInDeviceQueue0 " << newValue << std::endl;
// }

// void
// DevicePacketsInQueueTrace1 (uint32_t oldValue, uint32_t newValue)
// {
//   std::cout << "PacketsInDeviceQueue1 " << newValue << std::endl;
// }
...
int main (int argc, char *argv[])
{
    ...
    {
      queue_capacity = "20p"; // B, the total space on the buffer. doesn't work for now
    }
...
////////////////Install the QueueDisc on all the router NetDevices////////////////////////////////////////

    //Create a pointer to the queue-disc
    Ptr<QueueDisc> queueDisc = CreateObject<SharedBufferQueueDisc_v01> ();
    // Ptr<QueueDisc> queueDisc = CreateObject<FifoQueueDisc>();

    queueDisc->SetAttribute("MaxSize", QueueSizeValue (QueueSize("100p")));

    // Ptr<QueueDisc> queueDisc = CreateObjectWithAttributes<SharedBufferQueueDisc_v01> ("MaxSize", QueueSizeValue(QueueSize("100p"))); 

    // Create internal queue. currently un neccesarry
    // queueDisc->AddInternalQueue(
    //     CreateObjectWithAttributes<DropTailQueue<QueueDiscItem>>("MaxSize",
    //                                                                 QueueSizeValue(QueueSize("100p"))));

    //////////install the queue-disc on each NetDevice induvidualy    
    //Get the queue interfaces of the NetDevices
    Ptr<NetDevice> netDevice0 = switchDevicesOut.Get(0);
    Ptr<NetDevice> netDevice1 = switchDevicesOut.Get(1);

    Ptr<TrafficControlLayer> tc;
    
    tc = router.Get(0)->GetObject<TrafficControlLayer>();
    tc->SetRootQueueDiscOnDevice(netDevice0, queueDisc);
    tc->SetRootQueueDiscOnDevice(netDevice1, queueDisc);
    //call the initialize method of the queue-disc
    queueDisc->Initialize ();

    /////////// Alternativly use TrafficControlHelper to install queue-disc on all devices///////////
    //Create a TrafficControlHelper object:
    // TrafficControlHelper tch;
    //Add the NetDevices to the TrafficControlHelper object:
    //try:
    // tch.InstallMulti(switchDevicesOut, queueDisc);
    /////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    //     queue->TraceConnectWithoutContext ("PacketsInQueue", MakeBoundCallback (&TcPacketsInQueue_Trace, i));
    // }
    ...
    // Ptr<PointToPointNetDevice> p2pNDev0 = DynamicCast<PointToPointNetDevice>(netDevice0);
    // Ptr<PointToPointNetDevice> p2pNDev1 = DynamicCast<PointToPointNetDevice>(netDevice1);
    // Ptr<Queue<Packet>> queue0 = p2pNDev0->GetQueue();
    // Ptr<Queue<Packet>> queue1 = p2pNDev1->GetQueue();
    // queue0->TraceConnectWithoutContext ("PacketsInQueue", MakeBoundCallback (&DevicePacketsInQueueTrace0));
    // queue1->TraceConnectWithoutContext ("PacketsInQueue", MakeBoundCallback (&DevicePacketsInQueueTrace1));
/////////////////////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    // ///////UnInstall default queue-discs from all net devices:
    // TrafficControlHelper tch;
    // tch.Uninstall(switchDevicesOut);

    // /////////Install the QueueDisc on all the router NetDevices//////////////////
    // //Create a pointer to the queue-disc                                                          
    // // Ptr<QueueDisc> queueDisc = CreateObjectWithAttributes<FifoQueueDisc> ("MaxSize", QueueSizeValue(QueueSize("100p"))); 
    ...
    // /////// Alternativly use TrafficControlHelper to install queue-disc on all devices//////////////
    // //Create a TrafficControlHelper object:
    // // TrafficControlHelper tch; // already been defined erlier
    // //Add the NetDevices to the TrafficControlHelper object:
    // // tch.InstallMulti(switchDevicesOut, queueDisc);
    // //for debug:////////////////////////
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    //     std::cout << queueDisc->GetStats () << std::endl;
    // }
    ...
    //     testFlowStatistics << queueDisc->GetStats () << std::endl;
    // }
